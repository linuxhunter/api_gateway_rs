...
/// Final handler that processes the request after all middleware
pub struct FinalHandler<F>
where
    F: Fn(
            GatewayRequest,
        ) -> futures::future::BoxFuture<'static, Result<GatewayResponse, GatewayError>>
        + Send
        + Sync,
{
    handler: F,
...
/// Middleware registry for storing and retrieving middleware
...
impl MiddlewareRegistry {
    ...
    pub fn register<M: Middleware + 'static>(&mut self, middleware: M) {
        self.middlewares.push(Arc::new(middleware));
    }

    /// Get all registered middlewares
    pub fn get_all(&self) -> Vec<Arc<dyn Middleware>> {
        self.middlewares.clone()
    }

    /// Get middleware by name
    pub fn get_by_name(&self, name: &str) -> Option<Arc<dyn Middleware>> {
        self.middlewares.iter().find(|m| m.name() == name).cloned()
    }

    /// Create a middleware chain from the registry
    pub fn create_chain(&self) -> MiddlewareChain {
        // Create a new chain with the middlewares directly
        MiddlewareChain {
